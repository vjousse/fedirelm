module Fediverse.Mastodon.Entities.Account exposing (..)

import Fediverse.Entities.Account as FediverseAccount
import Fediverse.Mastodon.Entities.Emoji exposing (Emoji, emojiDecoder, emojiEncoder)
import Fediverse.Mastodon.Entities.Field exposing (Field, fieldDecoder, fieldEncoder, toField)
import Fediverse.Mastodon.Entities.Role exposing (Role, roleDecoder, roleEncoder)
import Fediverse.Mastodon.Entities.Source exposing (Source, sourceDecoder, sourceEncoder, toSource)
import Iso8601
...
accountDecoder =
    Decode.succeed Account
        ...
        |> Pipe.optional "mute_expires_at" (Decode.nullable Iso8601.decoder) Nothing
...
accountEncoder a =
    Encode.object
        ...
        , ( "mute_expires_at", a.muteExpiresAt |> Maybe.map Iso8601.encode |> Maybe.withDefault Encode.null )
...
toAccount self =
    ...
    -- Only difference with the generic Account type
    , group = Just self.group
    , header = self.header
    ...
    , locked = self.locked
    , moved = Nothing
    , muteExpiresAt = self.muteExpiresAt
